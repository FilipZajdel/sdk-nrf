...
void get_active_nodes(zb_uint8_t param)
{
	(void)param;
	zb_uint16_t short_addr;
	zb_uint8_t  ret;
	zb_bool_t   reset_itr = ZB_TRUE;

	if (!zb_nwk_get_nbr_active_changed()) {
		printk("No changes in active neighbors!\n");
	} else {
		printk(">>>>>>> nbt changed <<<<<<<\n");
	}

	do {
		ret = zb_nwk_get_nbr_active_next(&short_addr, reset_itr);
		reset_itr = ZB_FALSE;
		printk("(ret %u) active node 0x%.4x\n", ret, short_addr);
	} while ((ZB_NWK_NEIGHBOR_ACTIVE_ADDR_NOT_FOUND_EOT & ret) == 0);

	ZB_SCHEDULE_APP_ALARM(get_active_nodes, (zb_uint8_t)0, 5*ZB_TIME_ONE_SECOND);
}
